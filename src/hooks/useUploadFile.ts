...
const useUploadImage = () => {
    ...
    if (files) {
      Promise.allSettled(files)
        ...
        .catch((err) => {
          console.log(err);
          notification.error({
            message: 'Có lỗi xảy ra !! vui lòng lòng upload ảnh lại',
          });
        });
    } else {
      setImgListPreview([]);
  ...
  const handleDeleteImage = (id: number) => {
    const indexImagePreviewDeleted = imgListPreview.findIndex(
      (item) => item.id === id,
    );
    imgListPreview.splice(indexImagePreviewDeleted, 1);
    setImgListPreview([...imgListPreview]);
    if (fileList) {
      const indexFileDeleted = fileList.findIndex(
        (item) => item.lastModified === id,
      );
