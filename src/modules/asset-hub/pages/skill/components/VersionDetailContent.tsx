...
import type { SkillVersionDetail } from '../types';
  ...
  return (
    <div className="flex flex-col gap-4">
      <motion.div variants={fadeInUp} initial="hidden" animate="show" className={`${CARD_BASE} px-5 py-4`}>
        <div className="flex flex-col gap-2.5">
          ...
          {version.file && (
            <MetaRow
              label="Tệp"
              value={`${version.file.name ?? 'Skill package'}${version.file.size ? ` · ${version.file.size.toLocaleString('vi-VN')} bytes` : ''}`}
            />
          )}
          {version.reviewed_at && (
      ...
      {/* Guide first, then the artifact itself — a pending version shows its diff, a settled one
          its stored skill.md. Version detail is one of the two surfaces that carry the full guide. */}
      <ArtifactContentTabs
          ...
          version.state === 'pending' && comparison ? (
            ...
            <section>
              ...
              <pre className="max-h-[640px] overflow-auto whitespace-pre-wrap break-words rounded-xl bg-ah-pale p-4 font-mono text-[13px] text-ah-ink">
                {version.skill_md_content || 'Không có nội dung skill.md.'}
              </pre>
            </section>
          )
        }
