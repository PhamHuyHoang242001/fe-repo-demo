// Detail page: /asset-hub/prompt/:id. Single promptApi.detail() call → package + active_version + versions[].
...
//
// XSS BOUNDARY: DOMPurify strips dangerous HTML/attrs from the prompt.md source BEFORE react-markdown renders it.
...
import { detail as fetchDetail } from './api/promptApi';
import type { PromptPackageDetail } from './types';
...
const ErrorState: React.FC<{ status: number | null; onRetry: () => void }> = ({ status, onRetry }) => (
  <div className="flex flex-1 flex-col items-center justify-center gap-4">
    <p className="text-sm font-semibold text-ah-red">
      {status === 404 ? 'Prompt không tồn tại.' : 'Không thể tải chi tiết prompt.'}
    </p>
    {status !== 404 && (
      <Button
        type="primary"
    ...
    <Link to="/asset-hub/prompt" className="text-sm text-ah-green hover:text-ah-green-d">
      ← Quay lại danh sách
    </Link>
  </div>
);
...
const PromptDetail: React.FC = () => {
  ...
  const [pkg, setPkg] = useState<PromptPackageDetail | null>(null);
  ...
  // XSS boundary: sanitize the raw prompt.md before react-markdown renders it.
  const safeMd = useMemo(
    () => DOMPurify.sanitize(pkg?.active_version?.prompt_content ?? ''),
    [pkg],
  );

  const crumb = (
    <div className="mb-2 flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wider text-ah-muted">
      <Link to="/asset-hub/prompt" className="transition-colors hover:text-ah-green">Prompt</Link>
      <span>›</span>
      <span>#{id}</span>
    </div>
  );
  ...
  // yet pending/rejected-only own prompts still need Edit). Disabled while a pending version exists.
  return (
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {pkg.isUpdate && (
          <Button
            ...
            onClick={() => navigate(`/asset-hub/prompt/${id}/edit`)}
        ...
        <div className="flex min-w-0 flex-col gap-5">
          <Reveal>
            {safeMd ? (
              ...
              <div className="rounded-2xl border border-dashed border-ah-line bg-ah-pale px-5 py-4 text-sm text-ah-muted">
                Chưa có nội dung prompt.md.
              </div>
            )}
          </Reveal>
        </div>
