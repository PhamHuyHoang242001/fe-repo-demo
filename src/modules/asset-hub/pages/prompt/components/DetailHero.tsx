// Gradient hero band for the prompt detail page. Bold treatment: SURFACE_HERO gradient +
// shimmer sweep + framer-motion entrance. Avatar, name, version badge, actions.
// XSS boundary is upstream (PromptDetail sanitizes before passing pkg down).
  ...
  if (!v) {
    return (
      <div className="rounded-2xl border border-ah-line bg-ah-pale p-5 text-sm text-ah-muted">
        Prompt này chưa có phiên bản được duyệt.
      </div>
    );
  }
    ...
    >
      ...
      <div className="relative flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        ...
        <div className="shrink-0">
          <PromptActions packageId={pkg.id} promptContent={v.prompt_content} />
        </div>
      </div>
    </motion.div>
  );
