// Single card in the published prompt grid — "hero" treatment.
// Avatar: renders <img> when avatar_url is present; falls back to initials placeholder.
// Click navigates to /asset-hub/prompt/:id.
...
const PromptCard: React.FC<PromptCardProps> = ({ prompt }) => {
  ...
  return (
    <MotionCard
      ...
      onClick={() => navigate(`/asset-hub/prompt/${prompt.id}`)}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => e.key === 'Enter' && navigate(`/asset-hub/prompt/${prompt.id}`)}
    >
      ...
      {/* Footer meta — prompt length + last updated, pinned to baseline */}
      <div
        className={`flex items-center justify-between border-t border-ah-line pt-3 text-[10px] text-ah-muted ${v.tags.length > 0 ? '' : 'mt-auto'}`}
      >
        <span className="tabular-nums">{(v.prompt_content?.length ?? 0).toLocaleString()} ký tự</span>
